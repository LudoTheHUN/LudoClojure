...
(defn enqueue-kernel-withlocal
  ([kernel global-size local-size & args]
     (let [kernel ^CLKernel ((program) kernel)]
       (doseq [[idx arg] (indexed (map get-cl-object args))]
	 (.setArg kernel idx arg))
       (.enqueueNDRange kernel (local local-size)(queue) (to-dim-array global-size) *workgroup-size* (make-array CLEvent 0)))))
