...
liquidState2_b[gid] = 100.0;                     //Activation Potential getting set to the spike level, making another spike very unlikely untill it decays off
    ...
    for(iatom = 0; iatom < connections; iatom+=1 ) {
        ...
        gid_to_read = abs(random_value % gsize);           //Randomnumber generator step: which nuron to read from     TODO: add locality via xyz space here... Note this is the only place that needs to know the liquid conectivity topology, this can also be created procedurally.
...
(readoff [liquid spec] "read the liquid state as specified by the spec  WIP"
...
(def myliquid (make_liquid {:liquidsize (* 64 64) :connections 150}))
(time (do (dotimes [n 100] (flop myliquid))  (lg_finish ((:liquid_queue myliquid) @cl-utils/opencl_env))))
(do (dotimes [n 5] 
        (do (println n ":" (count (filter  (fn [x] (> x 0.1)) (readoff myliquid :foo))) (take 10 (readoff myliquid :foo)) )
...
(count (filter  (fn [x] (> x 0.0)) (readoff myliquid :foo)))

(take 20 (filter  (fn [x] (> x 0.0)) (readoff myliquid :foo)))
